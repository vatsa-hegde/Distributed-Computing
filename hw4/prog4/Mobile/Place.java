...
public class Place extends UnicastRemoteObject implements PlaceInterface {
    ...
    // InputStream configInput = new FileInputStream(System.getProperty("hazelcast.config"));
    // Config config = new XmlConfigBuilder(configInput).build();
	...
	throws RemoteException {
        try {
            ...
            System.out.println("Receiving agent " + classname + "...");


            // Deserialize agent
            Agent agent = deserialize(entity);
            System.out.println("Deserialized agent with ID: " + agent.getId());
            ...
            agent.setHazelcastInstance(hz); // inject Hazelcast instance


            //  Start agent thread
            Thread thread = new Thread(agent);
            thread.start();
            System.out.println("Started agent thread for: " + classname);
